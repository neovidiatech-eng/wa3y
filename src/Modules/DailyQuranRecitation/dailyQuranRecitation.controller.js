...
export const createDailyQuranRecitation = asyncHandler(async (req, res, next) => {
  ...
  const teacher = req.user.teacher || (await db.findOne({ model: "teacher", where: { user_id: req.user.id } }));
...
export const updateDailyQuranRecitation = asyncHandler(async (req, res, next) => {
  const { id } = req.params;
  const { studentId, surah, startPage, endPage, dueDate, status } = req.body;
  ...
  if (userRole === "student" || req.user.student) {
    const student = req.user.student || (await db.findOne({ model: "student", where: { user_id: req.user.id } }));
  ...
  const updatedRecitation = await db.updateOne({
    ...
    data: {
      ...
      ...(startPage !== undefined && !isStudentUpdating && { startPage: Number(startPage) }),
      ...(endPage !== undefined && !isStudentUpdating && { endPage: Number(endPage) }),
...
export const deleteDailyQuranRecitation = asyncHandler(async (req, res, next) => {
  const { id } = req.params;
...
export const getDailyQuranRecitationById = asyncHandler(async (req, res, next) => {
  const { id } = req.params;
...
export const getAllDailyQuranRecitations = asyncHandler(async (req, res, next) => {
  const { studentId, teacherId, status, surah, search, page = 1, limit = 10 } = req.query;
  ...
  const { items: recitations, pagination } = await db.findManyWithPaginationAndCount({
    model: "DailyQuranRecitation",
    where,
    page,
    limit,
...
export const getStudentDailyQuranRecitations = asyncHandler(async (req, res, next) => {
...
export const getTeacherDailyQuranRecitations = asyncHandler(async (req, res, next) => {
